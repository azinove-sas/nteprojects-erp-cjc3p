...
SUBMODULE_GITHUB=github.com/azinove-sas/packages

# .gitmodules submodule path
SUBMODULE_PATH=packages

GITHUB_ACCESS_TOKEN_SUBMODULE=$GITHUB_TOKEN

# github access token is necessary
# add it to Environment Variables on Vercel
if [ "$GITHUB_ACCESS_TOKEN_SUBMODULE" == "" ]; then
  echo "Error: GITHUB_ACCESS_TOKEN_SUBMODULE is empty"
  exit 1
fi
...
COMMIT="master" # get rid of the suffix
...
git remote add origin https://$GITHUB_ACCESS_TOKEN_SUBMODULE@$SUBMODULE_GITHUB # add origin of the submodule
